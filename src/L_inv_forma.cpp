...
void L_inv_forma::definirGeometria(float posX, float posY, int estado)
{
    ...
    if (estado == 1)
    {
        blocos[0].setPosition(sf::Vector2f(posX - 27.f, posY - 27.f));
        blocos[1].setPosition(sf::Vector2f(posX - 27.f, posY));
        blocos[2].setPosition(sf::Vector2f(posX, posY));
    ...
    {
        blocos[0].setPosition(sf::Vector2f(posX + 27.f, posY - 27.f));
        blocos[1].setPosition(sf::Vector2f(posX, posY - 27.f));
        blocos[2].setPosition(sf::Vector2f(posX, posY));
    ...
    if (estado_desejado == 1)
    {
        ...
        if (!TetrisJogo::disponivel(x + 1, y) && x > 1)
        {
            ...
            if (TetrisJogo::disponivel(x, y) && TetrisJogo::disponivel(x - 1, y) &&
                TetrisJogo::disponivel(x + 1, y) && TetrisJogo::disponivel(x - 1, y - 1))
            {
    ...
    if (estado_desejado == 2)
    {
        ...
        if (!TetrisJogo::disponivel(x, y + 1) && y > 1)
        {
            ...
            if (TetrisJogo::disponivel(x, y) && TetrisJogo::disponivel(x, y + 1) &&
                TetrisJogo::disponivel(x, y - 1) && TetrisJogo::disponivel(x + 1, y - 1))
            {
                definirGeometria(ponto_rotacao.x, ponto_rotacao.y, estado_desejado);
                cont_bloqueio = 0;
            }
