...
void S_forma::definirGeometria(float posX, float posY, int estado)
{
    ...
    if (estado == 1)
    {
        ...
        blocos[3].setPosition(sf::Vector2f(posX + 27.f, posY - 27.f));
    }
...
void S_forma::rotacionar()
{
    ...
    if (estado_desejado == 3)
    {
        ...
        if (!TetrisJogo::disponivel(x - 1, y) && x < 8)
        {
            ...
            if (TetrisJogo::disponivel(x, y) && TetrisJogo::disponivel(x + 1, y) &&
                TetrisJogo::disponivel(x - 1, y + 1) && TetrisJogo::disponivel(x , y + 1))
            {
                definirGeometria(ponto_rotacao.x, ponto_rotacao.y, estado_desejado);
                cont_bloqueio = 0;
            }
